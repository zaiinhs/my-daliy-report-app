    ...
    <div className="space-y-6">
      <div className="flex flex-col space-y-2">
        <h1 className="text-3xl font-bold tracking-tight">
          Salary Documentation | @zaiinhs
        </h1>
        <p className="text-muted-foreground">
          Track and visualize your income over time
        </p>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              For the period Nov 2024 - Mar 2025
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(totalIncome / salaryData.length)}
            </div>
            <p className="text-xs text-muted-foreground">
              Based on {salaryData.length} months of data
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(
                Math.max(...salaryData.map((item) => item.amount))
              )}
            </div>
            <p className="text-xs text-muted-foreground">
              {salaryData.length > 0 &&
                (() => {
                  const maxAmount = Math.max(
                    ...salaryData.map((item) => item.amount)
                  );
                  const maxEntry = salaryData.find(
                    (item) => item.amount === maxAmount
                  );
                  if (maxEntry) {
                    const [day, month, year] = maxEntry.date.split("/");
                    const date = new Date(`${year}-${month}-${day}`);
                    return `Recorded in ${date.toLocaleDateString("en-US", {
                      month: "short",
                      year: "numeric",
                    })}`;
                  }
            ...
            <TabsContent value="chart" className="space-y-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Visualization of your monthly income over time
                  </CardDescription>
                </CardHeader>
            ...
            <TabsContent value="table" className="space-y-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Detailed view of your monthly income
                  </CardDescription>
                </CardHeader>
                <CardContent>
